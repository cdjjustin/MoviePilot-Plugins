...
│   └── 002-第二章.mp3
└── 盗墓笔记/
    ├── 039.第二季.第002集.xxx.mp3   ← 扁平：文件名含「第X季.第N集」
    ├── 第一季/                       ← 或：一级子目录分季（同播客插件）
    │   └── 001.mp3
    └── 第二季/
        └── 015.mp3
...
季/集识别优先级（对齐「有声书播客」）：
1. 文件名：`第二季.第002集` / `第二季-015` / `S02E15`
2. 一级子目录：目录名含「第二季」则用该季号，否则按自然排序编号（如 CD1→S01）
3. 仍无集号时，整理阶段在该季内顺排
...
将本仓库地址配置到 MoviePilot **V3** 插件市场的第三方源后刷新市场，即可看到并安装「有声书刮削整理」（需 **≥ 3.0.10**）。
